...
class RioMap():
  # Constructor
  def __init__(self, data_path="src/static/estacoes_pluviometricas.csv") -> None:
    self.map_visualization = self.generate_rio_map(data_path)

  # Reading and transforming Alerta Rio data
  def generate_rio_map(self, data_path):
    df_estacoes = pd.read_csv(f'./{data_path}')
